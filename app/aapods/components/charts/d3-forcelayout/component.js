...
var svg = null, gnodes = null, node = null, link = null;
...
export default Ember.Component.extend({
  ...
  start: function () {
    ...
    link.exit().remove();
    gnodes = gnodes.data(force.nodes());
    gnodes.enter().append('g').classed('gnode', true);
  ...
  didInsertElement: function () {
    ...
    var id = this.$().attr('id');
    //created an svg element
    svg = d3.select("#" + id).attr('width', width).attr('height', height);
    gnodes = svg.selectAll('g.gnode');
    //node = svg.selectAll(".node");
    link = svg.selectAll(".link");
    //Added code
    //var a = {name: "a","group":1}, b = {name: "b", "group":2}, c = {name: "c", "group":3};
    nodes.push.apply(nodes, graph.nodes);
    links.push.apply(links, graph.links);
    //links.push();
    this.start();
